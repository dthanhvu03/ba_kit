...
- [ ] Given mô tả tiền điều kiện cụ thể, không mơ hồ, là **trạng thái** chứ không phải hành động
- [ ] When mô tả 1 hành động duy nhất của user, **không có `And` sau When**, không liệt kê chuỗi thao tác (bấm A → chọn B → nhập C)
- [ ] Then mô tả kết quả đo lường được, chỉ kiểm tra kết quả **liên quan tới hành động ở When**
- [ ] ≥ 3 AC lặp cùng Given → gom vào `Background`
- [ ] Nhiều AC cùng logic, khác dữ liệu (ngưỡng, bảng phí, phân quyền) → dùng `Scenario Outline + Examples` (xem `references/gherkin-advanced.md`)
...
- [ ] Edge case mang tính domain-specific của domain đang viết (vd EdTech: hết quota mentor; Thanh toán: trùng giao dịch, vượt hạn mức; TMĐT: hết hàng; Bảo hiểm: trong thời gian chờ)
...
- [ ] Không gộp nhiều scenario vào 1 AC (dấu hiệu: câu "nếu... thì..." bên trong Then/And)
- [ ] Không chứa mục thuộc Definition of Done: "đã review code", "đã test", "đã deploy" (xem `checklists/definition-of-ready-done.md`)
- [ ] Không dùng từ tuyệt đối không kiểm chứng được: "tất cả", "bất kỳ", "mọi", "luôn luôn"
...
- [ ] Có dòng "Readiness (DoR)" đánh giá ✅/⚠️/❌ (xem `checklists/definition-of-ready-done.md`)
...
### Readiness (DoR)
✅ Sẵn sàng / ⚠️ Còn thiếu: ... / ❌ Chưa sẵn sàng: ...
```
